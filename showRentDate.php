...
{
    $customerName = $_POST["findInputCustomerName"];

    $sqlSelect = "SELECT marka.marka, model.model, avtomobil.cena, klient.ime_k, klient.adres_k, naem.br_dni, avtomobil.id_avt
    from carrent.naem
    join carrent.klient
    on naem.id_klient = klient.id_klient
    join carrent.avtomobil
    on naem.id_avt = naem.id_avt
    join carrent.model
    on avtomobil.id_model = model.id_model
    join carrent.marka
    on model.id_marka = marka.id_marka
    where klient.ime_k = '{$customerName}'
    order by naem.data";
                    ...
                    <div class="carsListing col-centered">
                        ...
                        <div class="carsInfo">
                            ...
                            <ul>
                                ...
                                <li><i class="fa fa-info"></i><?php echo $result['ime_k'];?></li>
